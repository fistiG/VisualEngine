# Building Friday Night Funkin': Visual Engine

**Please note** that these instructions are for compiling/building the game. If you just want to play Visual Engine.

 ⋅ [latest stable release](https://github.com/ItzBlueBerries/VisualEngine/releases/latest)
...
## Dependencies
 1. [Install Haxe 4.1.5](https://haxe.org/download/version/4.1.5/). You should use 4.1.5 instead of the latest version because the latest version has some problems with Friday Night Funkin': Visual Engine.
...
2. `git clone https://github.com/ItzBlueBerries/VisualEngine.git`
3. `cd` into the source code: `cd VisualEngine`
4. (optional) If you want to build a specific version of Visual Engine, you can use `git checkout` to switch to it (i.e. `git checkout 1.0-VE`)
...
- The build will be in `VisualEngine/export/<target>/bin`, with `<target>` being the target you built to in the previous step. (i.e. `VisualEngine/export/windows/bin`)
