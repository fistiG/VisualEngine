...
import settings.VisualEngineData;
import other.GameJolt.GameJoltLogin;
import other.GameJolt.GameJoltAPI;
import flixel.FlxG;
...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		FlxG.save.bind('VisualEngine', 'FruitsyOG');

		VisualEngineData.initSave();
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			{
				// Get current version of Visual Engine
				...
				http.onData = function (data:String)
				{
					...
					returnedData[1] = data.substring(data.indexOf('-'), data.length);
				  	if (!MainMenuState.visualEngineVer.contains(returnedData[0].trim()) && !OutdatedSubState.leftState && MainMenuState.nightly == "")
					{
	...
	override function beatHit()
	{
		...
		switch (curBeat)
		{
			...
			case 5:
				if (Main.watermarks)
					createCoolText(['Visual Engine', 'by']);
				else
					createCoolText(['In Partnership', 'with']);
			case 7:
				if (Main.watermarks)
					{
						addMoreText('FruitsyOG');
						addMoreText('Nikerlo');
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxG.camera.flash(FlxColor.WHITE, 0.85);
			remove(credGroup);
			skippedIntro = true;
		}
	}
